...
var Application, Loans, People, _ref,
...
module.exports = LoansController = (function(_super) {
  ...
  LoansController.prototype.update = function(model, success, error) {
    ...
    return this.udpateModel(model, people, {
      ...
      error: function(model, err) {
        console.log(err);
        return this.publishEvent('error', err);
      }
    });
  };
...
document.onreadystatechange = function() {
  if (document.readyState === "complete" || document.readyState === "loaded") {
    return window.LoanShark = new Application({
      title: 'Loan Shark',
...
module.exports = Person = (function(_super) {
  ...
  Person.prototype.initialize = function() {
    ...
    if (!this.isNew()) {
      ...
      return this.addBountyListener();
    }
  };

  Person.prototype.addBountyListener = function() {
    return this.listenTo(Chaplin.mediator.loans, 'change remove add', function(model) {
      if (model.get('lendee_id') === this.get('_id')) {
        return this.calculateBounty();
      }
    });
  };
